...
## heap vs stack; Which types rely on which?
...
## Move => When the ownership is transfer to other variable
s1 = String::….
s2 = s1
...
## & vs &mut; Why you can't return reference of the variable inside the function?
...
A concept that let us refer to group of type instead of one specific type 
Ex. Option<T>

## trait; What does impl mean? What is the difference between fn f(x: impl A, y: impl A) and fn f<T: A>(x: T, y: T)
...
## lifetime parameter; how to declare and use it?
...
## closure; capturing references vs moving ownership; fn() -> A vs Fn() -> A vs FnMut() -> A vs FnOnce() -> A; what is the difference between closure and normal function?
...
FnOnce() -> A : Traits. Apply to func/closure that will be call once including the one that move ownership of data.
Fn() -> A : Traits. Apply to immutable func/closures.
FnMut() -> A : Traits. Apply to mutable func/closures.

- closure vs func

closure may contains data (by capturing/moving) but func contain no data.

## into_iter vs iter vs iter_mut; how iterator works?
...
## Smart Pointers; Box vs Rc vs RefCell; What are the pros and cons of using one
...
## mpsc (and many other concurrency tools)
...
## dyn; What is it?; dyn A vs impl A
...
## how to return polymorphic return type that has the same trait?

Declare the return type as a trait object using dyn.
- Box<dyn Trait>

# Post Book
## Future, async, await; how async/await differ from thread?
